...
const isActive = (history, path) => {
    if (history.location.pathname === path) {
        return { color: '#ff9900', fontWeight: 'bold' }
    } else {
        return { color: 'white' }
    }
}
    ...
    return (
        <div>
            <ul className="nav nav-tabs bg-primary">
                ...
                <li className="nav-item">
                    <Link className="nav-link" style={isActive(history, '/shop')} to="/cart">Cart <sup className="badge badge-danger">{itemTotal()}</sup> </Link>
                </li>
